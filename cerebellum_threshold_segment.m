function layer = cerebellum_threshold_segment(file)  
...
igl_in_c = imclose(igl_in, strel('disk',10));
igl_in = igl_in | igl_in_c;
%igl_in = imfill(igl_in, 'holes');
igl_in  = smallfill( igl_in, 25000 );
dwl_final = igl_in - c_final;
...
dwl_final = bwareafilt(dwl_final > 0,[10000 Inf]);
...
a_bin = bwareafilt(a_bin > 0,[100 Inf]);
%imshow(imoverlay(a_nf, bwperim(a_bin ),  [0 1 0]))


a_mask = imdilate(a_bin, strel('disk', 10));
...
image_cell = {a_nf, b_nf, c_nf, a_set_thresh, b_set_thresh}; 
...
T = table([layer_vals.Area]', [mean_im_cell{1}]', [mean_im_cell{2}]', [mean_im_cell{3}]', [mean_im_cell{4}]', [mean_im_cell{5}]');
T.Properties.VariableNames = {'pixel_area', 'p27_mean', 'DAPI_mean', 'NeuN_mean', 'p27_thresh_mean', 'DAPI_thresh_mean' };
