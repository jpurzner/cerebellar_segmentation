function layer = cerebellum_superpixels7(file)  
...
egl_phase = a_phasesym .* egl_bin;
egl_mask = imclose(egl_bin, strel('disk',5));
egl_mask = egl_mask | (egl_phase > 0.3);
egl_mask = imclose(egl_mask, strel('disk',5));
egl_mask = bwareafilt(egl_mask > 0,[5000 Inf]);
egl_mask = imclose(egl_mask, strel('disk',10));
egl_mask = imdilate(egl_mask, strel('disk',15));
...
b_bin = imdilate(b_bin, strel('disk', 2));
...
%a_norm(a_norm < 0) = 0; 
a_bin = thresh_by_area( a_norm, all_cerebellum, 0.15);
a_bin = bwareafilt(a_bin > 0,[2000 Inf]);


a_whole = im_get_overlap(egl_mask, a_nfg_h > 0.5, 0.01, 200);
a_whole = a_whole > 4; 
a_phase_bin = im_get_overlap(a_whole, a_phasesym > 0.3,  0.01, 200); 
% mask is set up by a combination of 3 approaches 
a_mask = a_whole | a_bin | a_phase_bin;
a_mask = imdilate(a_mask, strel('disk',10));
a_mask = bwareafilt(a_mask > 0,[20000 Inf]);
a_mask = imdilate(a_mask, strel('disk',10));
% I dont see a way of setting this threshold
a_final = (a_nfg .* a_mask) > 0.6;

%imshow(imoverlay(a_nf, bwperim(a_final), [0 1 0 ]))



b_mask = imdilate(a_mask, strel('disk',50));
b_final = (b_nfg .* b_mask) > 0.6;
b_final = bwareafilt(b_final > 0,[10000 Inf]);
b_final = b_final - a_final;
b_final = b_final == 1;
% residual cells inside iEGL
b_final = bwareafilt(b_final > 0,[2000 Inf]);
...
ab_final = a_final + b_final; 
b_final = b_final .* (1 - (ab_final & c_final)); 
a_final = a_final .* (1 - (ab_final & c_final));
ab_final = a_final + b_final; 
ab_minus_mask = bwareafilt(ab_final > 0,[0 10000]);
b_final = b_final .* (1 - ab_minus_mask); 
a_final = a_final .* (1 - ab_minus_mask); 
ab_final = a_final + b_final; 
...
b_norm(b_norm < 0) = 0;
b_bin = thresh_by_area( b_norm, all_cerebellum, 0.2 );
b_mask = b_bin .* (1- a_all_bin) .* (1- c_mask);
...
pia = imdilate(pia_bin, strel('disk',15));
...
iegl_in = all_cerebellum - a_all_mask;
iegl_in = (iegl_in | ml_bin | c_mask) .* (1- pia); 
%iegl_in = imdilate(iegl_in, strel('disk',1));
iegl_in = iegl_in .* (1- pia);
iegl_in = iegl_in | a_all_mask ;
iegl_in = imfill(iegl_in, 'holes');
...
csf_space = ((1- b_nfad)  > 0.6) .* all_cerebellum .* (1 - (a_norm > 0.2));
csf_space = csf_space .* pia_mask ;

% subtract the csf from the egl_in mask 
iegl_in = iegl_in .* (1 - csf_space);
...
%c_bin = dilate_bin(c_bin, 2, 5000);
ml_bin = dilate_bin(ml_bin, 4, 1000);
%dwl_bin = dilate_bin(dwl_bin, 6, 8000);

all_bin = a_final + b_final + c_final + ml_layer_mask + dwl_mask + pc_layer_bin;
%set_bin = zeros(size(all_bin));
%set_bin(all_cerebellum == 1) = 1;
set_bin =  (a_final * 2) + (b_final *3)  + (c_final * 4) + (ml_layer_mask * 5) + (dwl_mask * 6) + (pc_layer_bin *7) ;
set_bin = set_bin .* (all_bin == 1) ;
set_bin = set_bin + (all_bin > 1) ;
...
a_norm_m = a_norm; 
a_norm_m(a_bin == 1 & all_bin == 1 & set_bin == 2) = 1;
a_norm_m(~ismember(set_bin, [2 1 0])) = 0;  

b_norm_m = b_norm; 
b_norm_m(b_bin == 1 & all_bin == 1 & set_bin == 3) = 1;
b_norm_m(~ismember(set_bin, [3 1 0])) = 0;  

c_norm_m = c_norm; 
c_norm_m(c_bin == 1 & all_bin == 1 & set_bin == 4) = 1;
c_norm_m(~ismember(set_bin, [4 1 0])) = 0;   

ml_norm_m = ml_norm; 
ml_norm_m(ml_bin == 1 & all_bin == 1 & set_bin == 5) = 1;
ml_norm_m(~ismember(set_bin, [5 1 0])) = 0;  

dwl_norm_m = dwl_norm; 
dwl_norm_m(dwl_bin == 1 & all_bin == 1 & set_bin == 5) = 1;
dwl_norm_m(~ismember(set_bin, [6 1 0])) = 0;  
