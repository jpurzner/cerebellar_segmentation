function layer = cerebellum_threshold_segment2_with_dcn(file)
% 10x version of with-DCN cerebellar segmentation. Identical to
% cerebellum_threshold_segment2.m except:
%   - adds DCN class (label 8) for NeuN+ clusters within DWL interior
%   - saves raw uint8 labels file (_labels.tif) alongside RGB segments
%   - uses jet(9) explicit palette so DCN doesn't crash label2rgb
% At 10x: 1 px = 1.024 um. Scaling factor is implicitly 1 (no 'scaling' var).
% Channel order at 10x: page 1 = p27 (a), page 2 = DAPI (b), page 3 = NeuN (c).
...
c_bin = imclose(c_bin, strel('disk',10));
...
c_final = imclose(c_final, strel('disk', 5));
...
igl_in = imdilate(igl_in, strel('disk',5));
igl_in = imclose(igl_in, strel('disk',5));
...
ml_out = imdilate(ml_out, strel('disk', 10)); 
ml_out = imerode(ml_out, strel('disk', 5)); 
...
c_final = c_final == 1;
dwl_final = igl_in - c_final;
ml_out = imdilate(ml_out, strel('disk', 3)); 
...
igl_in_mask = imdilate(igl_in, strel('disk',30));
...
egl_strict = imdilate(egl_strict, strel('disk',10));
egl_strict = bwareafilt(egl_strict > 0,[5000 Inf]);



egl_mask = imdilate(egl_bin, strel('disk',10));
...
egl_thin = imdilate(egl_thin, strel('disk',3));

egl_mask = egl_mask | egl_thin;
egl_mask = bwareafilt(egl_mask > 0,[20000 Inf]);
egl_mask = imdilate(egl_mask, strel('disk',5));
...
a_bin_smooth_c = imclose(a_bin_smooth, strel('disk',3));
...
a_mask = imdilate(a_bin_smooth, strel('disk', 5));
a_thin = bwmorph(a_mask,'thin', Inf);
a_bin_25 = a_bin_25 | imdilate(a_thin, strel('disk', 1)); 
...
oegl_out = imdilate(a_thin, strel('disk', 5)) ...
    | imdilate(all_outline, strel('disk',20)) ...
    | a_bin;
oegl_out = imdilate(oegl_out, strel('disk',5));
oegl_out = smallfill( oegl_out, 1000000 );
oegl_out = imerode(oegl_out, strel('disk',5));
...
ml_layer_mask = imdilate(igl_in_mask, strel('disk', 10)) | imdilate(egl_strict, strel('disk', 40)); 
...
dwl_final = imdilate(dwl_final, strel('disk',40));
dwl_final = dwl_final .* (1-c_final);
...
iegl_out = iegl_out  > 0 ;
iegl_out_mask = imdilate(iegl_out, strel('disk',20));
...
oegl_in_mask = imdilate(oegl_in, strel('disk',20));
...
%% Detect Deep Cerebellar Nuclei (DCN): NeuN+ clusters within DWL — 10x version
% Same logic as 20x with-DCN. Scaling=1 here, so areas /4 vs 20x.
% DCN must be in DWL INTERIOR (eroded) so the close+fill cleanup can never
% spill into adjacent EGL/ML/IGL territory.
dcn_final = false(size(dwl_final));
if any(dwl_final(:))
    % At 10x, 5 px = 5.12 um interior margin (vs 10*scaling=20px=10.24um at 20x)
    dwl_interior = imerode(dwl_final, strel('disk', 5));
    ...
    if numel(neun_in_dwl) > 100
        ...
        thr_floor = 0.20;   % NeuN must clear 20% of dynamic range
        thr = max([thr_otsu, thr_mean, thr_floor]);

        dcn_seed = (c_nf > thr) & dwl_interior;
        % Areas: at 10x divide by 4 (scaling^2 ratio 20x:10x = 4:1)
        dcn_seed = bwareafilt(dcn_seed > 0, [125 Inf]);    % 500/4 = 125
        dcn_seed = imclose(dcn_seed, strel('disk', 3));    % 5*scaling/2 ≈ 3 px
        dcn_seed = imfill(dcn_seed, 'holes');

        % Re-clip to interior — close+fill above can push outside dwl_interior
        dcn_seed = dcn_seed & dwl_interior;
        dcn_final = bwareafilt(dcn_seed > 0, [500 Inf]);   % 2000/4 = 500
    end
end
...
set_bin(dcn_final == 1) = 8;   % NEW: DCN, between DWL and IGL priority
set_bin(c_final == 1) = 4;
set_bin(b_final == 1) = 3;
set_bin(a_final == 1) = 2;
...
col_map =  [1 1 1
            ...
            1 0 1];   % NEW: DCN — magenta
show_bin = set_bin; 
b_inc = imdilate(b_final, strel('disk', 8));
show_bin(b_inc == 1) = 3;      
...
% Save raw uint8 labels FIRST so we always have them even if label2rgb fails
imwrite(uint8(set_bin), [file(1:(end-4)) '_labels.tif'])
% Use explicit 9-color jet palette to handle DCN (label 8) without crashing
imwrite(label2rgb(set_bin, jet(9), 'k'),[file(1:(end-4)) '_segments.tif'])
...
region_names = {'all_cerebellum', 'iEGL', 'oEGL', 'IGL', 'ML', 'DWL', 'PCL', 'DCN'};
% Truncate or pad to match table height (some slides may have 0 DCN → N=7)
nrows = height(T);
