"""Train DINO head on ALL 19 slides with gold-boost (no holdout) — single
model for downstream predictions."""
...
def main():
    ...
    print(f"all-slide training on {len(cache_files)} caches")

    # detect gold slides (those that have a corrected label TIFF)
    gold_files = []
    weak_files = []
    ...
    print(f"  gold (boosted): {[f.stem.replace('_features','') for f in gold_files]}")
    print(f"  weak: {len(weak_files)} slides")

    weak_ds = PatchDataset(weak_files, samples_per_class=10000)
    if gold_files:
        gold_ds = PatchDataset(gold_files, samples_per_class=40000)   # 4× boost
