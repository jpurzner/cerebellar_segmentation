...
% Adjusting histogram to better represent gray tone range
% NumTiles is the main factor, can be adjusted to [horizontal aize,
% vertical size]
    function ad = adapt(read_file, contrast)
        ad = adapthisteq(mat2gray(read_file),'clipLimit', contrast, ...
            'Distribution','rayleigh', 'NumTiles', [100 100], 'Range', 'original');
    end 

a_nf = adapt(Neun, 0.01);
b_nf = adapt(dapi, 0.005);
c_nf = adapt(p27, 0.01);
...
% function to apply background subtraction and Gaussian filtering
    function  out = preprocess_image(in, num)
        out = background_subtract(in);
        out = imgaussfilt(out, num); % Check Gauss Filt Value
    end 

a_nfg = preprocess_image(a_nf, 8);
b_nfg = preprocess_image(b_nf, 8);
c_nfg = preprocess_image(c_nf, 8);
disp('gaussian filtering done')

% Anisotropic diffusion for phasesym
    function nfad = andif(file)
        nfad = anisodiff(file, 20, 20, 0.25, 1);
    end 
...
% Phase Symmetry and non-max suppression for identification
% Phasesym detects image features based on phase symmetry
    function [ps_nmxs, file_ps] = phase(file)
        [file_ps, orientation] = phasesym(file, 5, 6, 3, 2.5, 0.55, 2.0, 0);
        [ps_nmxs] = nonmaxsup(file_ps, orientation, 3);
    end

[~, a_phasesym] = phase(a_nfad);
disp('phase symmetry done')

% Compute threshold values for the layers (Otsu's method) - separates image
% into background and foreground
a_level = graythresh(a_nfg);
b_level = graythresh(b_nfg);
c_level = graythresh(c_nfg);

% Assigning layers to coloured channels (1=r, 2=g, 3=b)
% Creating composite colour image
col(:,:,1) = a_nfg;
col(:,:,2) = c_nfg;
col(:,:,3) = b_nfg;

% remove low intensity parts of image 
    function [nfg_h, nfg_l] = intensity(file, threshold)
        nfg_h = background_subtract(mat2gray(file, [threshold 1]));
        nfg_l = mat2gray(file, [0 threshold]); 
    end

[a_nfg_h, a_nfg_l] = intensity(a_nfg, a_level);
[b_nfg_h] = intensity(b_nfg, b_level);
[c_nfg_h, c_nfg_l] = intensity(c_nfg, c_level);
...
%pk = pk + mat2gray(a_pc);
pc_bin  = pk > 0.1;
pc_bin = bwareafilt(pc_bin > 0, [40*scaling Inf]);
...
all_cerebellum_large = imclose(all_cerebellum, strel('disk', 100*scaling));
all_cerebellum_large = imfill(all_cerebellum_large, 'holes');
all_cerebellum_large = all_cerebellum_large - all_cerebellum;
% all_cerebellum_large = imclose(all_cerebellum_large, strel('disk', 20*scaling));
% all_cerebellum_large = imfill(all_cerebellum_large, 'holes');
all_cerebellum_large = bwareafilt(all_cerebellum_large > 0, [5000*scaling Inf]);
...
% all_cerebellum = imclose(all_outline, strel('disk', 40*scaling));
all_cerebellum = imdilate(all_cerebellum, strel('disk', 100*scaling));
all_cerebellum = imfill(all_cerebellum, 'holes');
all_cerebellum = imerode(all_cerebellum, strel('disk', 130*scaling));
...
c_bin = bwareafilt(c_bin > 0, [5000*scaling Inf]);
c_bin = imclose(c_bin, strel('disk', 10*scaling));
c_bin = bwareafilt(c_bin > 0, [20000*scaling Inf]);
...
c_final = bwareafilt(c_final > 0, [200000*scaling Inf]);
...
% dwl_mask = bwareafilt(dwl_mask > 0, [4000*scaling Inf]);
...
igl_in = bwareafilt(igl_in > 0, [70000*scaling Inf]);
...
ml_out = bwareafilt(ml_out > 0, [30000*scaling Inf]);
...
dwl_final = imerode(dwl_final, strel('disk', 5*scaling));
dwl_final = imdilate(dwl_final, strel('disk', 10*scaling));
dwl_final = dwl_final .* (1-c_final);
dwl_final = bwareafilt(dwl_final > 0, [20000*scaling Inf]);


igl_in = c_final | dwl_final;
igl_in_c = imclose(igl_in, strel('disk', 20*scaling));
...
%% C_final
c_final = c_final .* (1 -  c_out);
c_final = imerode(c_final, strel('disk', 5));
c_final = imfill(c_final, 'holes');
c_final = imopen(c_final, strel('disk', 5));
c_final = c_final == 1;
...
pc_layer_bin = imdilate(pc_layer_bin, strel('disk', 7));
pc_layer_bin = imfill(pc_layer_bin, 'holes');
pc_layer_bin = imopen(pc_layer_bin, strel('disk', 5));
...
a_bin = bwareafilt(a_bin > 0, [10*scaling Inf]);
...
a_bin_smooth = bwareafilt(a_bin_smooth > 0, [200*scaling Inf]);
...
a_bin= a_bin_25 > 0.2;
a_bin= bwareafilt(a_bin_smooth > 0,[500*scaling Inf]);

[~, a_thin, ~] = edgelink(a_thin);
a_thin = filledgegaps(a_thin, 51);
a_thin = filledgegaps(a_thin, 101);
a_thin = bwareafilt(a_thin > 0,[100*scaling Inf]);
...
b_bin_smooth = imclose(b_bin_smooth, strel('disk', 2*scaling));
b_bin_smooth = smallfill(b_bin_smooth | b_bin, 200);
b_bin_smooth = bwareafilt(b_bin_smooth > 0, [100*scaling Inf]);
b_bin_whole = b_bin;
b_bin = b_bin_smooth;
...
deep_nuclei = bwareafilt(deep_nuclei > 0,[10000*scaling Inf]);
...
ml_layer_mask = bwareafilt(ml_layer_mask > 0,[20000*scaling Inf]);
...
%% DWL_final
dwl_final = igl_in - c_final;
dwl_final = bwareafilt(dwl_final > 0,[200000*scaling Inf]);
dwl_final = imdilate(dwl_final, strel('diamond', 10));
dwl_final = imfill(dwl_final, 'holes');
dwl_final = imclose(dwl_final, strel('disk', 5));
...
ml_in = bwareafilt(ml_in > 0, [5000*scaling Inf]);
ml_in = imdilate(ml_in, strel('disk', 10*scaling));

%ml_in = imclose(ml_in, strel('disk',2));
ml_in = imfill(ml_in, 'holes');
ml_in = bwareafilt(ml_in > 0, [20000*scaling Inf]);

ml_in = ml_in .* (1- a_bin); 
ml_in = ml_in == 1;
ml_in = bwareafilt(ml_in > 0, [200000*scaling Inf]);
ml_in = imfill(ml_in, 'holes');
...
iegl_out = iegl_out  > 0 ;
iegl_out = bwareafilt(iegl_out > 0, [50*scaling Inf]);
iegl_out = iegl_out | b_bin;
iegl_out = bwareafilt(iegl_out > 0, [100*scaling Inf]);
iegl_out =  iegl_out - oegl_in;
iegl_out = iegl_out  > 0 ;
iegl_out = imclose(iegl_out, strel('disk', 40*scaling));
iegl_out = imdilate(iegl_out, strel('disk', 40*scaling));
iegl_out  = iegl_out - a_bin; 
iegl_out = iegl_out  > 0 ;
iegl_out_mask = imdilate(iegl_out, strel('disk', 50*scaling));

% now dilate the oegl
oegl_in = bwareafilt(oegl_in > 0, [500*scaling Inf]);
oegl_in  = smallfill(oegl_in, 3000);
oegl_in = imclose(oegl_in, strel('disk', 40*scaling));
oegl_in  = smallfill(oegl_in, 500*scaling);
oegl_in_mask = imdilate(oegl_in, strel('disk', 50*scaling));
...
oegl_in = bwareafilt(oegl_in > 0, [200*scaling Inf]);
oegl_in  = smallfill( oegl_in, 3000*scaling);
iegl_out = iegl_out - egl_intersect; 
iegl_out = iegl_out == 1;

% very sligh dilation 
oegl_in = imdilate(oegl_in, strel('disk', 10*scaling));


%% A_final
a_final = a_bin .* oegl_in .* (1- ml_in) .* (1-igl_in) ; % strips the temporary G0 arrested cells in iEGL
a_final = imerode(a_final, strel('disk', 4));
a_final = imdilate(a_final, strel('disk', 5));
a_final = imfill(a_final, 'holes');
a_final = imopen(a_final, strel('disk', 10));

%% B_final
b_final = b_bin  .* (1- ml_in) .* (1-igl_in) ;  %.* (1- oegl_in);
b_final = imdilate(b_final, strel('disk', 10));
b_final = imfill(b_final, 'holes');
b_final = imopen(b_final, strel('disk', 15));
b_final = b_final == 1;
...
ml_final = imdilate(ml_final, strel('disk', 8));
ml_final = imfill(ml_final, 'holes');
ml_final = imopen(ml_final, strel('disk', 8));


%% PC_bin_filt
pc_bin_filt = pc_bin .* ml_final;
pc_bin_filt = imdilate(pc_bin_filt, strel('disk', 4));
    
clear  b_nfg_l c_nfg_l cerebellum_boundary boundary_y boundary_x

% a_final = imerode(a_final, strel('diamond', 20));
% a_final = imdilate(a_final, strel('diamond', 25));
% a_final = imdilate(a_final, strel('disk', 5));
% a_final = imfill(a_final, 'holes');
% a_final = imopen(a_final, strel('disk', 10));

% b_final = imerode(b_final, strel('diamond', 7));
% b_final = imdilate(b_final, strel('diamond', 10));
% b_final = imdilate(b_final, strel('disk', 5));
% b_final = imfill(b_final, 'holes');
% b_final = imopen(b_final, strel('disk', 15));

% c_final = imerode(c_final, strel('diamond', 15));
% c_final = imerode(c_final, strel('disk', 5));
% c_final = imfill(c_final, 'holes');
% c_final = imopen(c_final, strel('disk', 5));

% ml_final = imerode(ml_final, strel('diamond', 5));
% ml_final = imdilate(ml_final, strel('diamond', 2));
% ml_final = imdilate(ml_final, strel('disk', 7));
% ml_final = imfill(ml_final, 'holes');
% ml_final = imopen(ml_final, strel('disk', 8));

% dwl_final = imclose(dwl_final, strel('diamond', 10));
% dwl_final = imdilate(dwl_final, strel('diamond', 10));
% dwl_final = imfill(dwl_final, 'holes');
% dwl_final = imclose(dwl_final, strel('disk', 5));

% pc_layer_bin = imdilate(pc_layer_bin, strel('diamond', 5));
% pc_bin_filt = imdilate(pc_bin_filt, strel('disk', 4));
% pc_layer_bin = imdilate(pc_layer_bin, strel('disk', 7));
% pc_layer_bin = imfill(pc_layer_bin, 'holes');
% pc_layer_bin = imopen(pc_layer_bin, strel('disk', 5));
...
imwrite(label2rgb(set_bin, 'jet', 'k'),[file(1:(end-4)) '_segments_.tif'])

imwrite(double(Neun) .* double(a_bin),[file(1:(end-4)) '_a_iegl_only.tif'])
...
imwrite(imoverlay(mat2gray(Neun), bwperim(a_final), [1 0 0]),[file(1:(end-4)) '_a_overlay.tif'])
imwrite(imoverlay(b_nf, bwperim(a_final), [1 0 0]),[file(1:(end-4)) '_bnf_overlay.tif'])
imwrite(imoverlay(mat2gray(dapi), bwperim(a_final), [1 0 0]),[file(1:(end-4)) '_b_overlay.tif'])
...
image_cell = {a_nf, b_nf, c_nf, Neun, p27,  a_bin_1, b_bin_25}; 
