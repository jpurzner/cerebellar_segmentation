...
**All layer masks now exported** (commit `<latest>`):
`python/layer_masks/<slide>_<layer>.tif` — one uint8 binary TIFF per
layer per slide, 171 files total (9 layers × 19 slides):

| layer    | label(s) | meaning                    |
|----------|----------|----------------------------|
| iEGL     | 2        | inner external granule     |
| oEGL     | 3        | outer external granule     |
| EGL      | 2 ∪ 3    | combined EGL ribbon        |
| IGL      | 4        | internal granule           |
| ML       | 5        | molecular layer            |
| DWL      | 6        | deep white layer           |
| PCL      | 7        | Purkinje cell layer        |
| DCN      | 8        | deep cerebellar nuclei     |
| anomaly  | 9        | detected tear/bleach       |

Plus `_SUMMARY.txt` with per-slide pixel counts and % of image.

Generated by `python/dino/render_all_layer_masks.py`. Re-run after
any MATLAB pipeline change to refresh.

Boundary/skeleton variants (EGL only, for now) in
`python/egl_boundary_masks/` (could be extended to ML/IGL if needed —
see `render_egl_boundary_mask.py`).
