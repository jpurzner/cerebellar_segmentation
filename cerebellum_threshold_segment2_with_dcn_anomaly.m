function layer = cerebellum_threshold_segment2_with_dcn_anomaly(file, varargin)
% 10x version of with-DCN + anomaly + topology pipeline.
%
% Mirror of cerebellum_threshold_segment20x_with_dcn_anomaly with these
% adjustments for 10x microscopy:
%   - Channel order: page 1 = p27 (a), page 2 = DAPI (b), page 3 = NeuN (c)
%     (20x has page 2 = NeuN, page 3 = DAPI — opposite)
%   - pixel_size = 1.0239 um/px (vs 0.5119 at 20x)
%   - All um-based topology (pia_dist_um, line_len_um, etc.) recomputed
%     against the 10x pixel size.
%
...
% Channel order for 10x: page 1 = p27, page 2 = DAPI, page 3 = NeuN
% (20x has page 2 = NeuN, page 3 = DAPI — different physical layout)
a = imread(file,1);
b = imread(file, 2);   % DAPI at 10x
c = imread(file, 3);   % NeuN at 10x
...
pia_dist_um = bwdist(~all_cerebellum_orig) * 1.0239;   % distance in microns
...
line_len_px = round(line_len_um / 1.0239);
...
dist_from_dwl_dcn_um = bwdist(dwl_dcn_mask) * 1.0239;
...
if n_unassigned > 0
    ...
    max_dist_px = round(50 / 1.0239);    % ~98 px
...
orphan_max_area = round(500 / (1.0239^2));    % ~1909 px (500 um^2)
